...
def Test_fusion(img_test1,img_test2,addition_mode='Sum'):
    # IR_Encoder1 = nn.DataParallel(IR_Encoder()).to(device)
    # IR_Encoder1.load_state_dict(torch.load(
    #         "/media/b/F/000Multimodal/0zhaozixiang/IVIF-DIDFuse-main/Train_result/IRcoder_weight.pkl"
    #         )['weight'])
    
    VI_Encoder1 = nn.DataParallel(Encoder()).to(device)
    VI_Encoder1.load_state_dict(torch.load(
            "/Train_result_CNN/VIcoder_weight.pkl"
            )['weight'])
    
    AE_Decoder1 = nn.DataParallel(Decoder()).to(device)
    AE_Decoder1.load_state_dict(torch.load(
            "/Train_result_CNN/Decoder_weight.pkl"
            )['weight'])
    # IR_Encoder1.eval()
    VI_Encoder1.eval()
    AE_Decoder1.eval()
    ...
    with torch.no_grad():
        F_i1, F_i2,F_ib,F_id=VI_Encoder1(img_test1)
        F_v1, F_v2,F_vb,F_vd=VI_Encoder1(img_test2)

        # feature_2_vi,feature_B_vi, feature_vi
        # feature_2_ir,feature_B_ir, feature_ir
    ...
    with torch.no_grad():
       Out4, Out12, Out = AE_Decoder1(F_i1, F_v1, F_i2, F_v2, F_ib, F_id, F_vb, F_vd, img_test2)
     
    return Out, output_img(Out)