...
class Fusion(nn.Module):
    def __init__(self, in_channels, wave):
        ...
        self.high = TransformerBlock(in_channels,8, 2.66, False, 'WithBias')
        ...
        self.low = TransformerBlock(in_channels,8, 2.66, False, 'WithBias')
        self.idwt = IDWT_2D(wave)
        self.up = Upsample(in_channels)

    def forward(self, x1,x2):
        # print("l1.size",x1.shape)   # 32 32
        # print("l2.size",x2.shape)   # 16 16 
        b, c, h, w = x1.shape
        x_dwt = self.dwt(x1)       #16 16 
        ...
        # print("highf.size",highf.shape)
        # print("ll.size",ll.shape)
        ...
        # x2 = self.up(x2)
        # print("l1.size, x2.size",ll.shape, x2.shape)
        low=torch.cat([ll, x2], 1)
        low = self.convl(low)
        lowf=self.low(low)
        # print("lowf.size",lowf.shape)
        out = torch.cat((lowf, highf), 1)
        out_idwt = self.idwt(out)
        # print("out_idwt.size",out_idwt.shape)
        return out_idwt
...
class UNet(nn.Module):
    def __init__(self, in_channels, wave):
        ...
        self.avgpool1 = Downsample(in_channels)
        self.avgpool2 = Downsample(in_channels)
...
class HLFD(nn.Module):
    ...
    def forward(self, x):
        low = self.down(x)   #low frequence
        # print("low.size",low.shape)
        high = x - F.interpolate(low, size=x.size()[-2:], mode='bilinear', align_corners=True)  #high frequence
        # print("high.size",high.shape) 

        lowf=self.unet(low)
        # print("lowf.size",lowf.shape)

        highfeat = self.dense(high)
        # print("highfeat.size",highfeat.shape)
        lowfeat = F.interpolate(lowf, size=x.size()[-2:], mode='bilinear', align_corners=True)
        # print("lowfeat.size",lowfeat.shape)
